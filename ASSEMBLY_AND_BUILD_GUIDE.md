...
- [Battery Recommendations](#battery-recommendations)
- [Building Firmware](#building-firmware)
- [Dongle + Trackball Configuration](#dongle--trackball-configuration)
...
- **Soldering iron and supplies** (solder, flux, solder sucker/desoldering braid)
- **Tweezers and precision tools**
- **Multimeter** (for testing connections)
- **Patience and a well-lit workspace**

### Step 1: Prepare the PCB and Components

1. Inspect all components for damage before assembly
2. Test the nice!nano v2 microcontroller before soldering
3. Verify all diodes are oriented correctly (black stripe towards the same direction)
4. Check all switch sockets are clean and undamaged

### Step 2: Install Diodes

1. Insert diodes into the PCB (ensure proper orientation - black stripe marking)
2. Diodes should be on the **back** of the PCB
3. Fold and solder in place, then trim excess leads
4. The negative side (black stripe) should face towards the top of the PCB

### Step 3: Install the Microcontroller (nice!nano v2)

1. Solder headers to the nice!nano v2
2. Insert the nice!nano into the PCB socket
3. Ensure proper alignment with the silk screen markings
4. Double-check all pins are making good contact

### Step 4: Install Switch Sockets

1. Insert MX switch sockets into the PCB
2. Solder all pads on both sides of the socket
3. Ensure sockets are flush with the PCB surface
4. Test with a switch to verify proper fit

### Step 5: Install the Trackball Sensor (PMW3610)

> **Note:** This step applies to the right half only

1. Solder the PMW3610 sensor module to the designated pads
2. Verify pin alignment and orientation (check your specific PCB documentation)
3. Test continuity with a multimeter
4. The sensor connects through the custom input processor system

### Step 6: Install Stabilizers

1. Install stabilizers on spacebar and any 2u keys
2. Ensure stabilizers are properly clipped and secured
3. Mount stabilizer stems correctly before soldering if required

### Step 7: Assembly and Testing

1. **Pre-flashing Test**: Flash a simple test firmware to verify the microcontroller works
2. **Switch Test**: Install a few switches and test each key is responsive
3. **Trackball Test** (if applicable): Verify PMW3610 sensor responds
4. **Connection Test**: Check I2C/SPI lines with a multimeter if using display or sensor

### Step 8: Case Assembly

1. Place stabilizers and switches into the PCB (if not already done)
2. Install the PCB into the case
3. Mount the plate (if applicable)
4. Secure with screws or supports as designed
5. Install case feet or stands

### Step 9: Install Battery

See [Battery Recommendations](#battery-recommendations) for sizing guidance.

1. Connect the battery to the JST-PH 2.0 connector
2. Route the battery through the designated cable path
3. Secure the battery with double-sided tape in the allocated space
4. Ensure the battery does not put pressure on components

### Step 10: Final Assembly

1. Close the case and secure all fasteners
2. Install keycaps
3. Test all keys, layers, and trackball functionality
4. Verify Bluetooth connectivity (if using Bluetooth firmware)
...
⚠️ **IMPORTANT:** The PMW3610 trackball sensor consumes significant power. We **strongly recommend using a 1000mAh or larger battery** for the right half instead of a standard 500mAh battery.

#### Battery Comparison

| Capacity | Runtime | Notes |
|----------|---------|-------|
| **500mAh** | ~2-3 days | Insufficient for trackball usage; frequent charging required |
| **1000mAh** | ~7-10 days | **Recommended** for trackball configurations |
| **2000mAh+** | ~2-3 weeks | Fits in larger cases; may require case modification |
...
5. **Protection**: Batteries with PCM (protection circuit module) are recommended
...
- **Low-latency wireless** connection via nRF24L01+
- **USB connection** to computer
- **Shared configuration** between Bluetooth and Dongle modes
- **Input processors** for pointer, scrolling, and acceleration
...
| **Colemak DH** | `extra-keymaps/colemak_dh.keymap` | ✅ Included | Alternative typing layout |
| **Graphite** | `extra-keymaps/graphite.keymap` | ✅ Included | Modern typing layout |
| **Canary** | `extra-keymaps/canary.keymap` | ✅ Included | Optimized for finger comfort |
| **Focal** | `extra-keymaps/focal.keymap` | ✅ Included | Gaming-friendly layout |
...
Polish-specific keys include: Ą, Ć, Ę, Ł, Ń, Ó, Ś, Ź, Ż and their uppercase variants.
...
| # | Layer | Purpose |
|---|-------|---------|
| 0 | **BASE** | Standard typing with home-row mods |
| 1 | **NUM** | Numbers + function keys |
| 2 | **NAV** | Navigation, arrows, paging |
| 3 | **SYM** | Symbols and punctuation |
| 4 | **GAME** | Gaming (no modifiers) |
| 5 | **EXTRAS** | Shortcuts, Bluetooth controls |
| 6 | **MOUSE** | Full mouse layer (pointer + wheel) |
| 7 | **SLOW** | Low-speed pointer mode |
| 8 | **SCROLL** | Vertical/horizontal scrolling |

### Home-Row Mods

Home-row modifiers allow one-handed modifier access:

**Left Hand:**
- A = Alt
- S = Shift
- D = Ctrl
- F = Gui (Windows/Mac key)
...
- **Thumb cluster combos** for layer switching and mouse clicks
- **Double-tap and hold** for quick functions (backspace, mouse clicks)
- **Layer-specific behaviors** for gaming and accessibility
...
If you benefit from this firmware and the projects it builds upon, consider:

1. **Starring** the repositories on GitHub
2. **Contributing** improvements and bug fixes
3. **Reporting** issues to help improve the projects
4. **Sharing** your builds and feedback with the community
5. **Donating** to original authors if they support it
...
- **[ZMK Discord Community](https://discord.gg/8V9Wbgr)** - Active community for support and discussion
- **[nice!nano Documentation](https://nicekeyboards.com/)** - Microcontroller documentation
...
**License:** See the main repository for license information
